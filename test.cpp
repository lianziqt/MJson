...
{
	TEST_LITERAL(json::TYPE_NULL, "null");
	TEST_LITERAL(json::TYPE_TRUE, "true");
	TEST_LITERAL(json::TYPE_FALSE, "false");
}
...
#define TEST_NUMBER(expect, content)\
	do {\
		...
		EXPECT_EQ_BASE("parse ok", status);\
		EXPECT_EQ_BASE(json::TYPE_NUMBER, j.get_type());\
...
#define TEST_STRING(expect, content)\
	do {\
		...
		v.parse(content, status);\
		EXPECT_EQ_BASE("parse ok", status);\
		EXPECT_EQ_BASE(json::TYPE_STRING, v.get_type());\
...
static void test_parse_array() {
	...
	EXPECT_EQ_BASE("parse ok", status); 
	EXPECT_EQ_BASE(json::TYPE_ARRAY, v.get_type()); 
	...
	EXPECT_EQ_BASE(json::TYPE_NULL,   v.get_array_element(0).get_type());
	EXPECT_EQ_BASE(json::TYPE_FALSE,  v.get_array_element(1).get_type());
	EXPECT_EQ_BASE(json::TYPE_TRUE,   v.get_array_element(2).get_type());
	EXPECT_EQ_BASE(json::TYPE_NUMBER, v.get_array_element(3).get_type());
	EXPECT_EQ_BASE(json::TYPE_STRING, v.get_array_element(4).get_type());
	...
	for (size_t i = 0; i < 4; ++i) {
		M::Json a = v.get_array_element(i);
		EXPECT_EQ_BASE(json::TYPE_ARRAY, a.get_type());
		EXPECT_EQ_BASE(i, a.get_array_size());

		for (size_t j = 0; j < i; ++j) {
			M::Json e = a.get_array_element(j);
			//e.set_null();
			EXPECT_EQ_BASE(json::TYPE_NUMBER, e.get_type());
...
static void test_parse_object()
{
	...
	v.parse(" { } ", status);
	EXPECT_EQ_BASE("parse ok", status);
	EXPECT_EQ_BASE(json::TYPE_OBJECT, v.get_type());
	...
	EXPECT_EQ_BASE(json::TYPE_NULL, v.get_object_value(0).get_type());
	EXPECT_EQ_BASE("f", v.get_object_key(1));
	EXPECT_EQ_BASE(json::TYPE_FALSE, v.get_object_value(1).get_type());
	EXPECT_EQ_BASE("t", v.get_object_key(2));
	EXPECT_EQ_BASE(json::TYPE_TRUE, v.get_object_value(2).get_type());
	EXPECT_EQ_BASE("i", v.get_object_key(3));
	EXPECT_EQ_BASE(json::TYPE_NUMBER, v.get_object_value(3).get_type());
	EXPECT_EQ_BASE(123.0, v.get_object_value(3).get_number());
	EXPECT_EQ_BASE("s", v.get_object_key(4));
	EXPECT_EQ_BASE(json::TYPE_STRING, v.get_object_value(4).get_type());
	EXPECT_EQ_BASE("abc", v.get_object_value(4).get_string());
	EXPECT_EQ_BASE("a", v.get_object_key(5));
	EXPECT_EQ_BASE(json::TYPE_ARRAY, v.get_object_value(5).get_type());
	EXPECT_EQ_BASE(3, v.get_object_value(5).get_array_size());
	for (int i = 0; i < 3; ++i) {
		M::Json e = v.get_object_value(5).get_array_element(i);
		EXPECT_EQ_BASE(json::TYPE_NUMBER, e.get_type());
		EXPECT_EQ_BASE(i + 1.0, e.get_number());
	}
	EXPECT_EQ_BASE("o", v.get_object_key(6));
	{
		M::Json o = v.get_object_value(6);
		EXPECT_EQ_BASE(json::TYPE_OBJECT, o.get_type());
		for (int i = 0; i < 3; ++i) {
			M::Json ov = o.get_object_value(i);
			EXPECT_EQ_BASE('1' + i, (o.get_object_key(i))[0]);
			EXPECT_EQ_BASE(1, o.get_object_key_length(i));
			EXPECT_EQ_BASE(json::TYPE_NUMBER, ov.get_type());
...
#define TEST_ERROR(error, content) \
	do {\
		...
		EXPECT_EQ_BASE((json::TYPE_NULL), v.get_type());\
	} while(0)
...
static void test_access_boolean()
{
	...
	v.set_string("a");
	v.set_boolean(false);
	EXPECT_EQ_BASE(json::TYPE_FALSE, v.get_type());
}
...
static void test_access_object()
{
	...
	for (int j = 0; j <= 5; j += 5) {
		...
		for (int i = 0; i < 10; ++i) {
			std::string key = "a";
			key[0] += i;
			auto index = o.get_object_index(key);
	...
	auto index = o.get_object_index("j");
	EXPECT_EQ_BASE(1, static_cast<int>(index >= 0));
	o.remove_object_value(index);
	index = o.get_object_index("j");
	EXPECT_EQ_BASE(1, static_cast<int>(index == 9));
	EXPECT_EQ_BASE(9, o.get_object_size());

	index = o.get_object_index("a");
	EXPECT_EQ_BASE(1, static_cast<int>(index >= 0));
	o.remove_object_value(index);
	index = o.get_object_index("a");
	EXPECT_EQ_BASE(1, static_cast<int>(index == 8));
	EXPECT_EQ_BASE(8, o.get_object_size());

	for (int i = 0; i < 8; i++) {
		std::string key = "a";
		key[0] += i + 1;
		EXPECT_EQ_BASE((double)i + 1, o.get_object_value(o.get_object_index(key)).get_number());
	}

	v.set_string("Hello");
	o.set_object_value("World", v);

	index = o.get_object_index("World");
	EXPECT_EQ_BASE(1, static_cast<int>(index >= 0));
	v = o.get_object_value(index);
	EXPECT_EQ_STRING("Hello", v.get_string());
...
static void test_move() {
	...
	v2 = v1;
	v3 = std::move(v2);
	EXPECT_EQ_BASE(json::TYPE_NULL, v2.get_type());
